...
| A4.12 | In-memory adapter thread-safe: stress 1000 match concorrenti + 10000 mosse parallele → no `ConcurrentModificationException`, sequence ancora monotonico | `InMemoryRepositoryConcurrencyTest` (`@Tag("slow")`) — **DEFERRED** a REVIEW Fase 4 (non in IMPLEMENTA, scope sotto-fase TEST) |
...
| A4.18 | Regression: `RuleEngineCorpusTest` F1 (48 posizioni) + `AiTournamentSimulationTest` F2 (gating ≥95/100) continuano a passare | **DEFERRED** a sotto-fase TEST Fase 4 (regression `mvn clean verify` root con `slow,performance` inclusi) |
| A4.19 | `mvn clean verify` (root, **inclusi** `slow,performance`): BUILD SUCCESS su tutti i moduli           | **DEFERRED** a sotto-fase TEST Fase 4                                                |
