...
| A4.5.10    | `UiScalingService` 100/125/150 ortogonale al layout responsive                                                                                          | ✅ COVERED — REVIEW F-009 risolto via **compose** (commit `4086572`): `JavaFxScalingHelper` × `UiScalingService.activeScaleFactor()` testato da `JavaFxScalingHelperTest` 8 parametric (sceneWidth × uiScale → expected px) + `UiScalingServiceTest` 2 (activeScaleFactor sanitisation). I due meccanismi compongono per moltiplicazione, non si sovrascrivono. |
...
| A4.5.18    | `mvn clean verify` (root, slow + performance inclusi) BUILD SUCCESS                                                                                     | ✅ COVERED — TEST sotto-fase E5 (TEST-PLAN-fase-4.5 §5.2): BUILD SUCCESS in 18:16 min, **1019 test verdi** (391 shared + 166 core-server + 461 client + 1 server). Nessuna regression cross-modulo. |
