...
- **Ultimo task completato**: Task 3.1 — Bootstrap JavaFX + Spring Boot DI. Commit `51b7783` su `feature/3-ui-singleplayer`. `ClientApplication` (Spring Boot non-web), `JavaFxApp` (JavaFX entry point), `JavaFxAppContextHolder` (static bridge), `SceneRouter` (bean con API `initialize/show` e placeholder inline finché Task 3.2-3.6 non collegano gli FXML), `SceneId` enum (SPLASH, MAIN_MENU), `ClientProperties` record `@ConfigurationProperties("dama.client")`. `application.yml` minimal. `client/pom.xml` ora ha `<mainClass>com.damaitaliana.client.ClientApplication</mainClass>` per `mvn -pl client javafx:run`. `ClientSmokeTest` rimosso. `ClientBootstrapTest` con 4 test verdi. `mvn -am -pl client verify` BUILD SUCCESS in ~21s sul modulo client (shared+core-server build inclusi).
- **Prossimo task**: Task 3.2 — Design system: `theme-light.css` + `theme-dark.css` (stub) + `components.css`, font Inter caricato, `ThemeService` bean.
