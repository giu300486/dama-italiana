...
- **Ultimo task completato**: Task 3.3 — Localizzazione. Commit `8b859b8`. `MessageSourceConfig` (`@Configuration` con `ReloadableResourceBundleMessageSource`, basename `classpath:i18n/messages`, UTF-8, defaultLocale Italian, fallbackToSystemLocale=false). `LocaleService` bean (volatile Locale, default IT, `current()` / `switchTo(Locale)`). `I18n` helper bean (`t(code)` / `t(code, args...)` con bracketed-key fallback `[missing.key]`). `messages_it.properties` + `messages_en.properties` con seed minimo (app.title, locale.*, common.button.*, splash.loading) — future task aggiungeranno chiavi screen-specific. **Deviazione vs PLAN §4 Task 3.3**: il test `localeServiceSwitchPersistsPreference` è rinviato a Task 3.4 dove `PreferencesService` esiste; il piano documenta che l'ordine 3.3↔3.4 è scambiabile. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 27 test totali (4 bootstrap + 9 theme + 6 message-source + 4 locale + 4 i18n).
- **Prossimo task**: Task 3.4 — Preferences service: `UserPreferences` record, `PreferencesService` con atomic write `~/.dama-italiana/config.json`, integrazione con `LocaleService`.
