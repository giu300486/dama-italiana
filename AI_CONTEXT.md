...
- **Ultimo task completato**: Task 3.2 — Design system. Commit `6341f94`. `theme-light.css` con tutti i token SPEC §13.2 (palette, board, piece, highlight) come looked-up colors, 5 stati interattivi su `.button`/`.card`/`.text-field`/`.list-cell`, `.card-elevated` con dropshadow, `.popover`. `theme-dark.css` come stub (palette dark SPEC, non caricato in F3). `components.css` con classi board/piece/highlight/move-history. `ThemeService` bean con `stylesheetUrls()` testabile + `applyTheme(Scene)` + font loading best-effort. Inter TTF binaries non in repo (vedi `resources/fonts/README.md`): font-family chain garantisce fallback graceful su Segoe UI/Helvetica/sans-serif. `ThemeServiceTest` 9 metodi (plain JUnit, no TestFX): stylesheet ordering, classpath presence, tutti i color token, font-family chain, 5 stati interattivi button, dropshadow card-elevated, dark stub palette, component classes, optionality Inter. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS in ~30s, 13 test totali (4 + 9).
- **Prossimo task**: Task 3.3 — Localizzazione: `MessageSource` Spring + `messages_it/en.properties` skeleton, `LocaleService` bean.
