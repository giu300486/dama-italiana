...
- **Ultimo task completato**: Task 3.8 — Board renderer + side panel skeleton. Commit `c111c15`. Nuovo package `com.damaitaliana.client.ui.board`: `BoardRenderer extends Region` (8×8 grid of BoardCellNode children, layoutChildren via BoardLayoutMath con rank=0 in basso per SPEC §3.1, API `renderState(Board)`/`highlightLegalTargets(List<Square>)`/`highlightMandatorySources(...)`/`clearHighlights()`/`snapshot(int)`/`setOnCellClicked(Consumer<Square>)`), `BoardCellNode` package-private (light/dark style classes, mounta `PieceNode` solo su dark, toggle `.legal-target`/`.pulse-mandatory`), `PieceNode extends StackPane` (Circle body con `.piece`/`.piece-black`/`.piece-king`, anello interno per king, accessibleText via `PieceAccessibleText.describe`). Helper pure-logic testabili senza JavaFX runtime: `BoardLayoutMath` (cellSize/x/y/isDarkSquare), `HighlightState` POJO (legalTargets/mandatorySources sets immutabili), `PieceAccessibleText` (formato "WHITE_MAN at 12" English-only F3, localizzazione a11y in F11). `board-view.fxml`: BorderPane con BoardRenderer al centro + side-panel placeholder card-elevated + Back button. `BoardViewController` (`@Component @Scope("prototype")`): pull SinglePlayerGame da GameSession, renderState(board), fallback MAIN_MENU se sessione vuota. `SceneRouter` route BOARD → /fxml/board-view.fxml. Aggiunte chiavi i18n `board.sidepanel.placeholder`. 17 nuovi test (6 BoardLayoutMath + 7 HighlightState + 4 PieceAccessibleText). Visual sanity-check launch (splash → menu → setup → board → back) senza exception. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 85 test totali (+ Task 3.21 coprirà BoardRenderer/PieceNode/BoardCellNode JavaFX-side via TestFX).
- **Prossimo task**: Task 3.9 — Interazione board: `SinglePlayerController` orchestratore stato gioco, `MoveSelector` helper, click pedina → highlight legali, click target → applyMove → autosave + AI hook (mockati 3.13/3.16).
