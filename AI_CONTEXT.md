...
- **Ultimo task completato**: Task 3.6 — Main menu. Commit `d3baa79`. `SceneId` esteso con `SP_SETUP`, `RULES`, `SETTINGS` (placeholder inline finché 3.7/3.18/3.17 non li wirano). `SceneRouter` switch ora routa MAIN_MENU al nuovo FXML. `UserPromptService` interface + `JavaFxUserPromptService` `@Component` (Alert.showAndWait): astrazione yes/no per testabilità senza TestFX. `main-menu.fxml`: GridPane 2×3 con 5 card visibili (Single Player + Rules + Settings abilitate, LAN/Online disabled con tooltip i18n; Profile omessa in F3 — auth è F5/6); ogni card è `card-elevated` con FontIcon mdi2 (gamepad, lan, web, book-open, cog). `MainMenuController` (`@Component @Scope("prototype")`): @FXML initialize bind label via I18n + consume `sceneRouter.consumeAutosavePromptOnNext()` per prompt "Riprendi partita interrotta?". Resume mantiene il file (Task 3.16 lo caricherà), Discard cancella `_autosave.json`. Aggiunte chiavi i18n `menu.{singleplayer,lan,online,rules,settings}.{title,subtitle}`, `menu.{lan,online}.disabled`, `menu.open`, `autosave.prompt.{title,header,content,resume,discard}`. `MainMenuControllerTest` 6 test (3 navigation + 3 autosave decision logic). `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 50 test totali (4+9+4+6+6+9+6+6).
- **Prossimo task**: Task 3.7 — Single-player setup modal: `sp-setup.fxml` con scelta livello/colore/nome partita, `SinglePlayerSetupController`, `SinglePlayerGame` record per parametri partita.
