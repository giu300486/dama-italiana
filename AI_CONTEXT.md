...
- **Ultimo task completato**: Task 3.4 — Preferences service. Commit `2990102`. `UserPreferences` record schema-versioned (campi: schemaVersion, locale, themeId, uiScalePercent, firstLaunch + helper `defaults()`/`withLocale()`/`withFirstLaunch()`); JWT-encrypted field deliberatamente omesso fino a F5/6 (bump schemaVersion=2). `PreferencesService` bean: load() difensivo (missing/malformed/unknown-schema → defaults + log), save() atomico (tmp + Files.move ATOMIC_MOVE+REPLACE_EXISTING, fallback REPLACE_EXISTING su filesystem non supportati con log WARN), finally cleanup tmp. `LocaleService` ora richiede `PreferencesService` in constructor: init da `preferences.load().locale()`, switchTo persiste via `preferences.save(prefs.withLocale(...))`. Risolto deferred test `localeServiceSwitchPersistsPreference` da Task 3.3 + aggiunto `initialLocaleIsReadFromPreferencesWhenPresent`. `I18nTest` aggiornato a costruire LocaleService via `@TempDir`-backed PreferencesService. 9 nuovi test in `PreferencesServiceTest` (defaults-on-missing, round-trip, parent dir, malformed-json, unknown-schema, no-tmp-leak, overwrite, locale language-tag, null-locale tolerance). `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 38 test totali (4+9+4+6+6+9).
- **Prossimo task**: Task 3.5 — Splash screen: `splash.fxml` con logo placeholder + ProgressIndicator, `SplashController` con `Task<Void> bootstrap()` che inizializza ThemeService, PreferencesService, LocaleService, autosave-flag, sleep min 1.5s, route a main menu.
