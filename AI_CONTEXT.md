...
- **Ultimo task completato**: Task 3.12 — Status pane. Commit `efe7149`. `StatusPaneState` record (humanLabel, aiLabel, turnLabel/null, endgameLabel/null, sideToMove, ended). `StatusPaneViewModel` pure (compute(game, state) → StatusPaneState; chiavi lowercase enum-name `status.turn.<color>`, `status.endgame.<status>`, `setup.color.<color>`, `setup.level.<level>`; player line via template `status.player.color={0} ({1})`). `StatusPane extends VBox` con human/AI labels + HBox(chip + turnLabel) + endgame banner che subentra quando ended; pure JavaFX. `SinglePlayerController` ora ha `setStateChangeListener(Consumer<GameState>)` + `fireStateChange()` invocato dopo start() (initial frame) e dopo ogni applyMove. `BoardViewController` wire listener: `gameController.setStateChangeListener(state -> statusPane.update(viewModel.compute(game, state)))`. Aggiunte chiavi i18n IT/EN: `status.player.{human,ai,color}`, `status.turn.{white,black}`, `status.endgame.{white_wins,black_wins,draw_repetition,draw_forty_moves,draw_agreement}` — copre tutte le 5 non-ongoing GameStatus. CSS `.side-chip` round 12px + `.side-chip-white`/`.side-chip-black`. `MessageSourceConfig.buildMessageSource()` promosso a `public` per consentire `StatusPaneViewModelTest` di costruire MessageSource senza Spring context. 11 nuovi test (player labels IT/EN, turn binding, endgame visibility, parametrizzato sui 6 GameStatus). `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 134 test totali.
- **Prossimo task**: Task 3.13 — Integrazione IA: `AiTurnService` (wraps `VirtualThreadAiExecutor`), wiring in `SinglePlayerController` per scheduling turno IA dopo mossa umana, integrazione animazioni (Task 3.10) nel flow di applyMove.
