...
- **Ultimo task completato**: Task 3.10 — Animation engine. Commit `1c9ee3e`. Nuovo package `com.damaitaliana.client.ui.board.animation`. `MoveAnimator` (stateless factory) con primitive SPEC §13.3: `slideMove` (TranslateTransition 250ms EASE_OUT), `fadeCapture` (ParallelTransition Scale+Fade 200ms), `promotion` (ParallelTransition Rotate 500ms Y-axis + SequentialTransition flash in/out), `pulseMandatory` (FadeTransition 800ms INDEFINITE auto-reverse). Tutti i parametri (durate, interpolatore, opacity floor) sono `public static final` per coerenza SPEC↔test. **Deviazione vs SPEC §13.3 documentata in package-info**: pulse usa `FadeTransition` non `FillTransition` — `FillTransition` richiede `Shape`, le cell sono `Region` senza fill property; il pulse cambia opacity per produrre l'equivalente visivo sul background-color. `AnimationOrchestrator` (stateless utility) compone Move → Animation: SimpleMove → singolo TranslateTransition; CaptureSequence → SequentialTransition di leg, ogni leg è ParallelTransition(slide-for-this-leg, fadeCapture-for-the-captured); moving piece Node risolto una volta all'inizio così byX/byY incrementali compongono il path completo. **Wiring in `SinglePlayerController.applyMove` rinviato a Task 3.13** (continuation async naturale per AI scheduling) — documentato in package-info. 15 nuovi test (7 MoveAnimator + 8 AnimationOrchestrator) plain JUnit + `javafx.scene.shape.Rectangle` placeholder. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 114 test totali.
- **Prossimo task**: Task 3.11 — Move history pane: `MoveHistoryView` (ListView<MoveHistoryRow>), `MoveHistoryViewModel` (ObservableList rows con `appendMove(Move, Color)` formattando via `FidNotation.format`), integrazione in `board-view.fxml`.
