...
- **Ultimo task completato**: Task 3.14 — Save format + `SaveService`. Pacchetto `client.persistence`: `SavedGame` record schema-versioned (`schemaVersion=1`, `kind="SINGLE_PLAYER_GAME"`, `name`, `createdAt`/`updatedAt` come `Instant`, `aiLevel`, `humanColor`, `moves` lista FID, `currentState: SerializedGameState`); factory `SavedGame.of(SinglePlayerGame, createdAt, updatedAt)`. `SerializedGameState` riusa lo schema corpus F1 (4 liste FID disgiunte `whiteMen`/`whiteKings`/`blackMen`/`blackKings`, ADR-022) + `sideToMove` + `halfmoveClock` + `history` FID; `fromState`/`toState` con replay-da-initial quando history non vuota (rebuild di `List<Move>` per repetition detection — F3 SP parte sempre da `GameState.initial()`). `SaveService` `@Component` su `ClientProperties.savesDir()` (default `~/.dama-italiana/saves/`); `save(slot, data)` con atomic write tmp+`Files.move(ATOMIC_MOVE, REPLACE_EXISTING)` (ADR-032) + fallback `REPLACE_EXISTING` su WARN (`AtomicMoveNotSupportedException`); `load(slot)` valida `schemaVersion` e lancia `UnknownSchemaVersionException` typed (A3.20); `listSlots()` ordina per `updatedAt` desc, esclude slot riservato `_autosave` e skippa slot con schemaVersion ignota o file corrotti (WARN); `delete(slot)` idempotente; `slugify(name)` static — NFD + strip diacritici + lower + collapse `[^a-z0-9]+`→`-` + trim, idempotente (es. "Domèni  co!!" → "domeni-co"); `requireValidSlot` regex `^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$` rifiuta input pericolosi (`../`, spazi, maiuscole). `SaveSlotMetadata` record (slot/name/aiLevel/humanColor/updatedAt/currentMoveNumber). `ObjectMapper` con `JavaTimeModule` (jsr310 transitivo da `spring-boot-starter`) + `WRITE_DATES_AS_TIMESTAMPS=false` per ISO-8601 di `Instant`. **17 nuovi test** in `SaveServiceTest`: roundtrip, listSlots ordinato desc, autosave escluso da listSlots, delete (+idempotente per missing), missing slot empty, schema version reject (typed exception + listSlots skip), no leftover .tmp, overwrite atomico, slot regex reject, slugify (idempotente + unicode/spaces + reject all-whitespace), 8-thread concurrent save, SerializedGameState round-trip dopo applyMove, SavedGame.of wiring, malformed JSON → UncheckedIOException. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, **161 test totali**, JaCoCo gate `client` 60% line+branch ✅, SpotBugs 0 High, Spotless OK.
- **Fix post-Task 3.13** (commit `83cf4b8`, baseline pre-3.14): cablate le animazioni dell'engine Task 3.10 in `SinglePlayerController.applyMove`. `BoardRenderer` espone `pieceAt(Square)` e `currentCellSize()`. Mocked renderer ritorna null → fast path sincrono → test esistenti invariati. Aggiunto `stopped` flag per droppare callback animazione tardive.
- **Prossimo task**: Task 3.15 — Save dialog + Load screen (FXML `save-dialog.fxml`/`load-screen.fxml`, `SaveDialogController`, `LoadScreenController`, miniatura board snapshot via `BoardRenderer.snapshot()`, integrazione `MenuBar` "Partita" → Salva/Carica/Termina). Dipendenze: Task 3.8, 3.14 (entrambi chiusi).
