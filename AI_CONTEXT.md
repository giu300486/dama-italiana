...
- **Ultimo task completato**: Task 3.9 — Interazione board. Commit `b02c018`. `MoveSelector` helper puro (filtra `RuleEngine.legalMoves` per `Move::from`). `AutosaveTrigger` interface (contract per Task 3.16; Optional<AutosaveTrigger> nel controller, empty in produzione finché 3.16 non lo wired). `SinglePlayerController` (`@Component @Scope("prototype")`): possiede `GameState` live + `Square selected`, `start(game, renderer)` wira `renderer.setOnCellClicked(this::onCellClicked)`, `onCellClicked` (package-private testabile) implementa il click protocol — (1) se selected esiste e click è target legale → applyMove; (2) se click è own piece → seleziona + highlight legali; (3) altrimenti deseleziona; click su same-selected → toggle off. `applyMove` aggiorna state, repaint renderer, refreshMandatoryHighlights, fire `autosaveTrigger.onMoveApplied(snapshot())`. `refreshMandatoryHighlights` calcola pezzi con cattura disponibile dopo ogni mossa. Forward deps animation (3.10) e AI scheduling (3.13) **non** wired in 3.9 — applyMove crescerà quando arriveranno. `GameDomainConfig` espone `@Bean RuleEngine = new ItalianRuleEngine()`. `BoardViewController` ora usa `ObjectProvider<SinglePlayerController>` per ottenere prototype fresco e chiamare `start(game, boardRenderer)`. 14 nuovi test (4 MoveSelector + 10 SinglePlayerController con Mockito mock di BoardRenderer, RuleEngine reale, Optional<AutosaveTrigger> testato sia presente che assente). Visual sanity-check launch pulito. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 99 test totali.
- **Prossimo task**: Task 3.10 — Animation engine: `MoveAnimator` con `slideMove` (TranslateTransition 250ms OUT_QUAD), `fadeCapture` (parallel scale+fade 200ms), `promotion` (rotate 500ms + flash dorato), `pulseMandatory` (FillTransition cyclic 800ms), `AnimationOrchestrator` per comporre sequenze.
