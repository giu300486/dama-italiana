...
- **Ultimo task completato**: Task 3.7 — Single-player setup modal. Commit `2120da3`. Pre-fix: launch test ha trovato `mdi2-checkerboard` errato in splash.fxml (Ikonli MD2 vuole `mdi2{first-letter}-{name}`) — corretto a `mdi2c-checkerboard` in commit `de1be0c`. App si avvia ora pulita, splash → main menu OK. Per Task 3.7: nuovo package `com.damaitaliana.client.controller` con `ColorChoice` enum (WHITE/BLACK/RANDOM con `resolve(rng)` deterministico-by-seed), `SinglePlayerGame` record (`AiLevel` + `Color` + name + `GameState` + `RandomGenerator`) con factory `tryCreate(...)` Optional che valida nome non-blank, `GameSession` bean volatile single-game-holder per passaggio cross-scene. `SceneId.BOARD` aggiunto (placeholder). `sp-setup.fxml` con 2 `ToggleGroup` (livello default Esperto, colore default Bianco), nome partita pre-popolato con "Partita del DD MMM HH:mm" localizzato via `DateTimeFormatter.localizedBy(locale)`, error label inline, pulsanti Annulla/Conferma. `SinglePlayerSetupController`: onConfirm → tryCreate → setCurrentGame → show(BOARD); onCancel → MAIN_MENU. Aggiunte chiavi i18n `setup.{title,level.*,color.*,name.*,button.confirm,error.name.empty}`. 18 nuovi test (6 ColorChoice + 5 GameSession + 7 SinglePlayerGame). `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 68 test totali (4+9+6+5+7+4+6+6+9+6+6). App rilanciata e startup pulito (no exception).
- **Prossimo task**: Task 3.8 — Board renderer + side panel skeleton: `BoardRenderer` (custom Region) + `BoardCellNode` + `PieceNode` + `BoardViewController` con `board-view.fxml`.
