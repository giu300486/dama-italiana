...
- **Fix post-Task 3.13** (commit `83cf4b8`): cablate le animazioni dell'engine Task 3.10 in `SinglePlayerController.applyMove` (deviazione precedente risolta). `BoardRenderer` espone `pieceAt(Square)` e `currentCellSize()`. `applyMove` se `pieceAt` non-null → `AnimationOrchestrator.animateMove` + `setOnFinished` → `finalizeMove` (state mutation + render). Mocked renderer ritorna null → fast path sincrono → test esistenti invariati. Aggiunto `stopped` flag per droppare callback animazione tardive. 144 test green.
- **Ultimo task completato**: Task 3.13 — Integrazione IA. Commit `f5ab6dc`. `AiTurnService` `@Component` `AutoCloseable` wraps `VirtualThreadAiExecutor`; `requestMove(state, level, rng)` builds `AiEngine.forLevel`, submits con per-level `DEFAULT_TIMEOUT` (PRINCIPIANTE 500ms / ESPERTO 2000ms / CAMPIONE 5000ms), bridge Submission → `CompletableFuture<Move>` su virtual thread "ai-turn-await". Cancellazione CF → graceful cancel sull'underlying search. `cancelAll()` cancella pending; `@PreDestroy close()` hard-cancel + executor shutdown. `AiThinkingState` POJO con `Consumer<Boolean>` listener (no JavaFX BooleanProperty per leggerezza). `SinglePlayerController` con 3° arg `Optional<AiTurnService>`, Executor `fxExecutor` (default `Platform::runLater`, override `Runnable::run` nei test), busy flag + pendingAiRequest. Dopo applyMove: `scheduleAiTurnIfAi()` quando sideToMove != humanColor → busy=true, thinking=true, `whenCompleteAsync(..., fxExecutor)`. `onCellClicked` no-op se busy. `stop()` cancella pending CF + reset flag (chiamato da BoardViewController.onBack). `BoardViewController` wire `aiThinkingState.onChange(thinking → statusPane.setThinking(thinking, label))`. `StatusPane` con ProgressIndicator + label nascosti quando idle. Chiavi i18n IT/EN `status.thinking`. **Animazioni Task 3.10 ancora un-wired** (BoardRenderer non espone per-square node lookup). 7 nuovi test `AiTurnServiceTest` (Principiante legal-move, terminal null, parallel independent, cancelAll graceful, close shutdown, null NPE, near-instant submission). 3 nuovi test `SinglePlayerControllerTest` (AI scheduled, click ignored mentre thinking, stop cancels pending). Test esistenti aggiornati per Optional.empty del 3° arg. Visual sanity-check launch pulito. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 144 test totali.
- **Prossimo task**: Task 3.14 — Save format + SaveService (`SavedGame` record schema-versioned, `SaveService` con atomic write, slug filename, listSlots ordinabile, miniatura snapshot).
