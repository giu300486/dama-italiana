...
- **Ultimo task completato**: Task 3.5 — Splash screen. Commit `701f4f6`. `SceneRouter` evoluto: ora prende `ApplicationContext` + `ThemeService` in constructor; `show(SceneId)` dispatcha a FXML loader via `setControllerFactory(context::getBean)` per gli ID con FXML wired (oggi solo SPLASH), placeholder inline per gli altri (MAIN_MENU finché Task 3.6); `themeService.applyTheme(scene)` su ogni show. Aggiunto flag one-shot `setAutosavePromptOnNext(boolean)` / `consumeAutosavePromptOnNext()` per il prompt "riprendi partita". `splash.fxml` con `org.kordamp.ikonli.javafx.FontIcon` (mdi2-checkerboard 96px), titolo `app.title`, label `splash.loading`, `ProgressIndicator`. `SplashController` (`@Component @Scope("prototype")`): @FXML initialize lancia `Task<BootstrapResult>` su virtual thread "splash-bootstrap"; `bootstrap()` (package-private testabile) detecta autosave + sleep min 1.5s SPEC §9.1; `finish()` setta flag + show(MAIN_MENU) — chiamato sul FX thread via `Task.setOnSucceeded`. `SplashControllerTest` 6 test diretti con SceneRouter mockato + `@TempDir`. `mvn -pl client verify -DexcludedGroups=slow,performance` BUILD SUCCESS, 44 test totali (4+9+4+6+6+9+6).
- **Prossimo task**: Task 3.6 — Main menu: `main-menu.fxml` con 6 card (Single Player abilitata, LAN/Online disabilitate con tooltip, Regole abilitata, Profilo nascosta, Impostazioni abilitata), `MainMenuController` che consuma `consumeAutosavePromptOnNext()` per mostrare prompt "Riprendi partita interrotta?".
