...
- **Decisione**: File JSON con campi top-level: `schemaVersion` (intero, attualmente `1`), `kind` (`"manual"` | `"autosave"`), `name` (string user-facing), `createdAt` / `updatedAt` (ISO-8601 UTC), `aiLevel` (`"PRINCIPIANTE"` | `"ESPERTO"` | `"CAMPIONE"`), `humanColor` (`"WHITE"` | `"BLACK"`), `currentState` (oggetto annidato: `whiteMen`, `whiteKings`, `blackMen`, `blackKings` — quattro liste FID disgiunte; `sideToMove`, `halfmoveClock`, `history` — lista di mosse FID-encoded). `SaveService.load` rifiuta `schemaVersion != 1` con `UnknownSchemaVersionException` (eccezione tipata, propagata fino a un toast localizzato).
- **Conseguenze**:
